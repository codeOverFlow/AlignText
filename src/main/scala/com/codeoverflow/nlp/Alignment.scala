...
object Alignment {
  ...
  def isCognate(w0: String, w1: String) =
    !isTransfugee(w0, w1) && w0.substring(0, 4) == w1.substring(0, 4) && bigram_feature(w0, w1) > 0.75 && trigram_feature(w0, w1) > 0.75
  ...
  def trigram_feature(w0: String, w1: String): Double = {
    var bi_w0 = List[String]()
    (1 until w0.length - 1).foreach { i =>
      bi_w0 = w0.substring(i - 1, i + 2) :: bi_w0
    }
    var bi_w1 = List[String]()
    (1 until w1.length - 1).foreach { i =>
      bi_w1 = w1.substring(i - 1, i + 2) :: bi_w1
    }
    (2.0 * bi_w0.intersect(bi_w1).length) / bi_w0.union(bi_w1).length
  }
}
